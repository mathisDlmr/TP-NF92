    ...
    <div class="wrapper">
        ...
        <main>
            ...
            <form method='POST' action='consulter_eleve.php'>
                <?php
                //On récupère tous les élèves qui ont été notés sur des séances passées
                include 'connexion.php';
                $recuperer_eleves_notes = "SELECT * FROM eleves WHERE ideleve IN (SELECT ideleve FROM inscription WHERE note!=-1 AND idseance IN (SELECT idseance FROM seances WHERE DateSeance < CURDATE())) ORDER by nom";
                $result = mysqli_query($connect, $recuperer_eleves_notes);
                ...
                //On vérifie qu'il existe des élèves notés à des séances futures
                if(mysqli_num_rows($result) == 0){
                    echo "Aucun élève n'a été noté";
                    echo "<a href='validation_seance.php' class='retour'>Noter un élève</a>";
                    mysqli_close($connect);
                    exit();
                };

                //On génère un formulaire à partir de tous les élèves notés dans la BDD
