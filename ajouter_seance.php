<!DOCTYPE html>
<html>

<head>
    <meta charset="UTF-8">
    <title>Ajouter séance</title>
    <link href="styles/style.css" rel="stylesheet">
    <link rel="icon" type="image/svg+xml" href="https://c0.klipartz.com/pngpicture/308/494/gratis-png-lightning-mcqueen-mater-doc-hudson-cars-la-compania-de-walt-disney-cars.png">
</head>

<body>
    <div class="wrapper">
        <header>
            <h1>Auto-école de l'UTC</h1>
            <nav>
                <ul>
                    <li><a href="accueil.html">Accueil</a></li>
                    <li class="deroulant"><a href="#">Gestion élèves &ensp;</a>
                        <ul class="sous">
                            <li><a href="ajout_eleve.html">Ajouter un élève</a></li>
                            <li><a href="consultation_eleve.php">Consultation élèves</a></li>
                            <li><a href="visualisation_calendrier_eleve.php">Calendrier élève</a></li>
                        </ul>
                    </li>
                    <li class="deroulant"><a href="#">Gestion thèmes &ensp;</a>
                        <ul class="sous">
                            <li><a href="ajout_theme.html">Ajouter un thème</a></li>
                            <li><a href="suppression_theme.php">Supprimer un thème</a></li>
                        </ul>
                    </li>
                    <li class="deroulant"><a href="#">Gestion séances &ensp;</a>
                        <ul class="sous">
                            <li><a href="ajout_seance.php">Ajouter une séance</a></li>
                            <li><a href="inscription_eleve.php">Inscrire un élève à une séance</a></li>
                            <li><a href="desinscription_seance.php">Désinscrire un élève à une séance</a></li>
                            <li><a href="validation_seance.php">Valider une séance</a></li>
                        </ul>
                    </li>
                </ul>
            </nav>
        </header>
        <main>
            <h2>Ajouter une séance</h2>
            <?php
            //On vérifie que les vérifications HTML "required" n'ont pas été supprimées
            if (empty($_POST['date']) || empty($_POST['theme']) || empty($_POST['effMax'])) {
                echo "<p>Un problème a été rencontré avec vos données. <i>(Champ vide)</i></p>";
                echo "<a href='ajout_seance.php' class='retour'>Retour</a>";
                exit();
            };

            //On vérifie que les vérifications HTML "pattern" n'ont pas été supprimées
            if (!preg_match("/^[0-9]+$/", $_POST['effMax']) || !preg_match("/^\d{4}-\d{2}-\d{2}$/", $_POST['date'])) {           /// ^=Dès le premier caractère , [0-9]=caractèrs acceptés , + = motif répété >0 fois , $=jusqu'au dernier caractère , d{4}=excatement 4 chiffres
                echo "<p>Un problème a été rencontré avec vos données. <i>(Mauvais format)</i></p>";
                echo "<a href='ajout_seance.php' class='retour'>Retour</a>";
                exit();
            };

            //On vérifie que les vérifications HTML "max" n'ont pas été supprimées
            if ($_POST['effMax']>50) { 
                echo "<p>Un problème a été rencontré avec vos données. <i>(Effectif dépassé)</i></p>";
                echo "<a href='ajout_seance.php' class='retour'>Retour</a>";
                exit();
            };

            //On vérifie que les vérifications HTML sur la date n'ont pas été supprimées
            date_default_timezone_set('Europe/Paris');
            $date = date("Y\-m\-d");
            if (strtotime($_POST['date'])<strtotime($date)) { 
                echo "<p>Un problème a été rencontré avec vos données. <i>(Date passée)</i></p>";
                echo "<a href='ajout_seance.php' class='retour'>Retour</a>";
                exit();
            };

            //Si les vérifications HTML sont respectées, on vérifie maintenant que l'idtheme n'a pas été modifié
            include 'connexion.php';
            $verif_theme_existe = 'SELECT * FROM themes WHERE idtheme= "'.$_POST['theme'].'"';
            $result = mysqli_query($connect, $verif_theme_existe);

            if (!$result) {
                echo "<br>Erreur de connexion à la BDD  ".mysqli_error($connect);
                echo "<a href='ajout_seance.php' class='retour'>Retour</a>";
                exit();
            };

            //Si le thème n'est pas trouvé, alors l'idtheme a été modifié
            if (empty(mysqli_fetch_array($result))) {
                echo "<p>Un problème a été rencontré avec vos données. <i>(Thème inexistant)</i></p>";
                echo "<a href='ajout_seance.php' class='retour'>Retour</a>";
                mysqli_close($connect);
                exit();
            };

            //Si les vérifications HTML sont respectées, on vérifie que la séance n'existe pas déjà 
            $verif_seance_inexistante = 'SELECT * FROM seances WHERE DateSeance = "'.$_POST['date'].'" AND idtheme= "'.$_POST['theme'].'"';
            $result = mysqli_query($connect, $verif_seance_inexistante);

            if (!$result) {
                echo "<br>Erreur de connexion à la BDD  ".mysqli_error($connect);
                echo "<a href='ajout_seance.php' class='retour'>Retour</a>";
                exit();
            };

            //Si la séance existe déjà, on ne la rajoute pas
            if (!empty(mysqli_fetch_array($result))) {
                echo "<p>Vous ne pouvez pas créer 2 séances avec le même thème à la même date</p>";
                echo "<a href='ajout_seance.php' class='retour'>Retour</a>";
                mysqli_close($connect);
                exit();
            };

            //Si tout est bon, on rentre la séance dans la BDD
            $rentrer_seance = 'INSERT INTO seances VALUES (NULL,"'.$_POST['date'].'", "'.$_POST['effMax'].'", "'.$_POST['theme'].'")';
            $result = mysqli_query($connect, $rentrer_seance);

            if (!$result) {
                echo "<br>Erreur de connexion à la BDD  ".mysqli_error($connect);
                echo "<a href='ajout_seance.php' class='retour'>Retour</a>";
                exit();
            };

            echo "<p>La séance a bien été ajoutée ! </p>";
            echo "<a href='ajout_seance.php' class='retour'>Retour</a>";

            mysqli_close($connect);
            ?>
        </main>
    </div>
    <footer>
        <a target="_blank" href="https://www.utc.fr/" class="lien-icone"><img src="https://upload.wikimedia.org/wikipedia/fr/thumb/4/45/Logo_UTC_2018.svg/2560px-Logo_UTC_2018.svg.png" alt="UTC" height="42px"></a>
        <div>
            <a target="_blank" href="https://facebook.com/" class="lien-icone"><img src="https://logodownload.org/wp-content/uploads/2014/09/facebook-logo-1-2.png" alt="Twitter" height="42px" width="42px"></a>
            <a target="_blank" href="https://instagram.com" class="lien-icone"><img src="https://clipartcraft.com/images/instagram-logo-transparent-background-2.png" alt="Instagram" height="42px" width="42px"></a>
        </div>
    </footer>
</body>

</html>