...
<body>
    <div class="wrapper">
        ...
        <main>   
            ...
            if (empty($_POST['theme']) || empty($_POST['descriptif'])) {
                echo "<p>Un problème a été rencontré avec vos données. <i>(Champ vide)</i></p>";
                echo "<a href='ajout_theme.html' class='retour'>Retour</a>";
                exit();
            };
            ...
            $theme_echappe = mysqli_real_escape_string($connect, $_POST['theme']);
            $descriptif_echappe = mysqli_real_escape_string($connect, $_POST['descriptif']);

            //On vérifie que l'utilisateur n'a pas rafraichit la page (que le thème n'est pas déjà activé)
            $verif_theme_inexistant = 'SELECT * FROM themes WHERE nom = "'.$theme_echappe.'" and supprime = 0';
            $result = mysqli_query($connect, $verif_theme_inexistant);
            ...
            //Si le thème est déjà existant et activé, l'utilisateur a rafraichit la page -> on bloque la nouvelle entrée
            if(!empty(mysqli_fetch_assoc($result))){
                echo "<p>Ce thème a déjà été entré dans la base de données</p>";
                echo "<a href='ajout_theme.html' class='retour'>Retour</a>";
                mysqli_close($connect);
                exit();
            };

            //On regarde maintenant si le thème existe mais est simplement désactivé
            $verif_theme_desactive = 'SELECT * FROM themes WHERE nom = "'.$theme_echappe.'" and supprime = 1 ';
            $result = mysqli_query($connect, $verif_theme_desactive);
            ...
            //Si le thème a été désactivé, on le réactive
            if (!empty(mysqli_fetch_array($result))) {
                $query = 'UPDATE themes SET supprime = 0 WHERE nom = "'.$theme_echappe.'"';
                ...
                echo "<p>Le thème a été réactivé !</p>";
                echo "<a href='ajout_theme.html' class='retour'>Retour</a>";
                mysqli_close($connect);
                exit();
            };

            //Si le thème n'est pas trouvé dans la BDD, alors on le créé
            $valider_theme = 'INSERT INTO themes VALUES (NULL,"'.$theme_echappe.'", 0 , "'.$descriptif_echappe.'")';
            $result = mysqli_query($connect, $valider_theme);
            ...
            echo "<p>Le thème a été ajouté !</p>";
            echo "<a href='ajout_theme.html' class='retour'>Retour</a>";
