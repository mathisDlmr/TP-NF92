...
<body>
    <div class="wrapper">
        ...
        <main>
            ...
            //Si les vérifications HTML sont respectées, on vérifie que la page n'a pas été rafraichie
            $verif_eleve_inexistant = 'SELECT * FROM eleves WHERE nom="'.$nom_echappe.'" AND prenom="'.$prenom_echappe.'" AND dateNaiss="'.$_POST['naissance'].'"';
            ...
            //Si l'élève est déjà rentré (=> l'utilisateur a rafraichit la page), on ne rajoute pas l'élève (on suppose qu'il n'y aura pas 2 élèves avec le même nom, prénom et date de naissance)
            if(!empty(mysqli_fetch_array($result))){
                echo "<p>Cet élève a déjà été entré dans la base de données</p>";
                echo "<a href='ajout_eleve.html' class='retour'>Retour</a>";
                mysqli_close($connect);
                exit();
            };

            //Si tout est bon, on ajoute l'élève
