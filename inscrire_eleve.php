    ...
    <div class="wrapper">
        ...
        <main>
            <h2>Inscription séance</h2>
                ...
                if ((empty($_POST['ideleve'])) || (empty($_POST['idseance']))) {
                ...
                $verif_eleve_existe = 'SELECT * FROM eleves WHERE ideleve = "'.$_POST['ideleve'].'"';
                ...
                $verif_seance_existe = 'SELECT * FROM seances WHERE idseance = "'.$_POST['idseance'].'"';
                $result = mysqli_query($connect, $verif_seance_existe);
            
                //Si l'idseance n'existe pas, on en informe l'utilisateur
                if(empty(mysqli_fetch_array($result))){
                ...
                //On vérifie que l'élève n'est pas déjà inscrit à la séance
                $verif_eleve_pas_inscrit = "SELECT * FROM inscription WHERE idseance=$seance AND ideleve=$eleve";
                $result = mysqli_query($connect, $verif_eleve_pas_inscrit);
                ...
                //Si l'elève est déjà inscrit (l'ideleve a été modifié), on refuse l'entrée dans la BDD
                if (!empty(mysqli_fetch_array($result))) {
                    echo "<p>Cet élève a déjà été inscrit</p>";
                    echo "<a href='inscription_eleve.php' class='retour'>Retour</a>";
                    mysqli_close($connect);
                    exit();
                };

                //Si tout est bon, on ajoute l'élève à la séance
                $inscrire_eleve = "INSERT INTO inscription VALUES ($seance, $eleve, DEFAULT)";
                $result = mysqli_query($connect, $inscrire_eleve);
                ...
                echo "<p>L'élève a été inscrit ! </p>";
                echo "<a href='inscription_eleve.php' class='retour'>Retour</a>";
